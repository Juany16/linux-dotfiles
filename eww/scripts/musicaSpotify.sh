...
# playerctl metadata --format '{{playerName}}|{{status}}|{{title}}|{{artist}}' -F 2>/dev/null | while read -r line; do

    # Jika tidak ada player aktif, line mungkin kosong atau error
    if [ -z "$line" ]; then
        echo '{"title": "Sin Reproductor", "artist": "~", "status": "Stopped", "player": "", "cover": ""}'
        # echo '{"title": "Sin Reproductor", "artist": "~", "status": "Stopped", "player": ""}'
        continue
    fi
    ...
    # cover_path=$("$player" "$art_url" "$file_path")
    ...
    # echo "{\"player\": \"$player\", \"status\": \"$status\", \"title\": \"$title\", \"artist\": \"$artist\"}"
